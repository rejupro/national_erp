...
<section class="content-header">
   <h1>@if ( Auth::User()->language == 1 ) লন রিসিভ @else Loan Receive @endif</h1>
   <ol class="breadcrumb">
      <li><a href="{{route('home')}}"><i class="fa fa-dashboard"></i>@if ( Auth::User()->language == 1 ) ড্যাসবোর্ড @else Dashboard @endif</a></li>
      <li class=""><a href="">@if ( Auth::User()->language == 1 ) লন রিসিভ @else Loan Receive @endif</a></li>
   </ol>
</section>
<!-- Main content -->
      ...
      <div class="col-md-12">
         <div class="box box-solid">
            <div class="box-header with-border">
               <h3 class="box-title">@if ( Auth::User()->language == 1 ) লন রিসিভ @else Loan Receive @endif</h3>
            </div>
            <div class="box-body">
               ...
               <form id="dynamic_form"    enctype="multipart/form-data" method="post" accept-charset="utf-8">
                @csrf
                  <div class="col-md-12 popup_details_div">
                     <div class="row">
                        <center>
                           <h3 class="page-title">@if ( Auth::User()->language == 1 ) লন রিসিভ ভাউচার @else LOAN RECEIVE VOUCHER @endif</h3>
                        </center>
                     </div>
                     <div id='result'></div>
                     <br>
    ...
    function dynamic_field(number)
    {
     ...
     html = '<tr>';
      html += '<th style="width:200px;"><select class="form-control select2" name="receive_to[]"><option selected value="">-No Source-</option><option  value="LE_5">Cash</option>@foreach($bankAccount as $account)<option value="BA_{{$account->id}}">ACC-{{$account->acc_no}}</option>@endforeach @foreach($mobile as $mobiles)<option value="MA_{{$mobiles->id}}">{{$mobiles->mname}}-{{$mobiles->mobile}}</option>@endforeach</select></th>';
           ...
           html += '<th style="width:200px;"><select  class="form-control select2" name="receive_from[]" id="pay_to" required><option selected value="">-Select-</option><optgroup label="Loan Invoice">@foreach($loaninvoice as $data)<option style="background-color:ash;" value="LO_{{$data->id}}">{{$data->inv_no}}-{{$data->total}}</option>@endforeach</optgroup></select></th>';
