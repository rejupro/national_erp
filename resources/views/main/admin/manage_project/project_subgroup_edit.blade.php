...
@section("content")  
    <section class="content-header">
        <h1>@if ( Auth::User()->language == 1 ) প্রোজেক্ট সাব-গ্রুপ ইডিট @else Project Sub-Group Edit @endif</h1>
        <ol class="breadcrumb">
     ...
     <section class="content">
        ...
        <h3 class="box-title">@if ( Auth::User()->language == 1 ) প্রোজেক্ট সাব-গ্রুপ ইডিট @else Project Sub-Group Edit @endif</h3>
        </div>
        <div class="box-body">
            @if ($errors->any())
        ...
        <form action="{{route('edit-prosubgroup-update',$subgroup->id)}}"  enctype="multipart/form-data" method="post" accept-charset="utf-8">
         @csrf
            ...
            <div class="row"style="margin-top: 15px" >
                ...
                <input type="submit" name="save_subgroup" id="submit" class="btn btn-flat bg-purple btn-sm " value="@if ( Auth::User()->language == 1 ) আপডেট @else Update @endif"/> <a href="{{route('prosubgroup-list-create')}}" class="btn btn-flat bg-gray  ">@if ( Auth::User()->language == 1 ) ক্লোজ @else Close @endif</a>
                </div> 
            </div>     
        </form>    
        </div>
