...
<section class="content-header">
   <h1>@if ( Auth::User()->language == 1 ) পেইমেন্ট ক্রিয়েট @else Payment Create @endif</h1>
   <ol class="breadcrumb">
      <li><a href="{{route('home')}}"><i class="fa fa-dashboard"></i>@if ( Auth::User()->language == 1 ) ড্যাসবোর্ড @else Dashboard @endif</a></li>
      <li class=""><a href="">@if ( Auth::User()->language == 1 ) পেইমেন্ট ক্রিয়েট @else Payment Create @endif</a></li>
   </ol>
</section>
<!-- Main content -->
      ...
      <div class="col-md-12">
         <div class="box box-solid">
            <div class="box-header with-border">
               <h3 class="box-title">@if ( Auth::User()->language == 1 ) পেইমেন্ট ক্রিয়েট @else Payment Create @endif</h3>
            </div>
            <div class="box-body">
               ...
               <form id="dynamic_form"    enctype="multipart/form-data" method="post" accept-charset="utf-8">
                @csrf
                  <div class="col-md-12 popup_details_div">
                     <div class="row">
                        <center>
                           <h3 class="page-title">@if ( Auth::User()->language == 1 ) পেইমেন্ট ভাউচার @else PAYMENT VOUCHER @endif</h3>
                        </center>
                     </div>
                     <div id='result'></div>
                     <br>
                     <div class="row">
                        <div class="col-md-4">
                           ...
                           {{-- <div class="form-group" >
                              <div class="input-group">
                                 <span class="input-group-addon"><b>Requisition:</b></span>
                                 <select class="form-control select2" name="r_id" >
                                    <option value="">-Select-</option>
                                    @foreach($requisition as $data)
                                    <option value="{{$data->id}}" @if(old('code') == $data->id) selected @endif>{{$data->code}}</option>
                                    @endforeach
                                 </select>
                              </div>
                           </div> --}}
                  ...
                  <div class="row"style="margin-top: 15px" >
                     ...
                     <div class="col-md-4 text-right" >
                        <input type="submit" name="save_journal" id="submit" class="btn btn-flat bg-purple btn-sm" value="@if ( Auth::User()->language == 1 ) সেভ @else Save @endif"/> <a href="{{ route('paymentvouchar-list')}}" class="btn btn-flat bg-gray ">@if ( Auth::User()->language == 1 ) ক্লোজ @else Close @endif</a>
                     </div>
                  </div>
               </form>
            </div>
...
<script>
   ...
   $(document).ready(function(){
    ...
    $('#dynamic_form').on('submit', function(event){
           ...
           $.ajax({
               url:'{{ route("paymentlist-store-route") }}',
               method:'POST',
               data:$(this).serialize(),
               dataType:'json',
               beforeSend:function(){
               ...
               success:function(data)
               {
                   ...
                   else
                   {
                     ...
                     //   $('#result').html('<div class="alert alert-success">'+data.success+'</div>');
                       window.location.href = "{{ route('paymentvouchar-list')}}";
                   }
