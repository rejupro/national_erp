...
<section class="content-header">
   <h1>@if ( Auth::User()->language == 1 ) জার্নাল ক্রিয়েট @else Journal Create @endif</h1>
   <ol class="breadcrumb">
      <li><a href="{{route('home')}}"><i class="fa fa-dashboard"></i>@if ( Auth::User()->language == 1 ) ড্যাসবোর্ড @else Dashboard @endif</a></li>
      <li class=""><a href="">@if ( Auth::User()->language == 1 ) জার্নাল ক্রিয়েট @else Journal Create @endif</a></li>
   </ol>
</section>
<!-- Main content -->
      ...
      <div class="col-md-12">
         <div class="box box-solid">
            <div class="box-header with-border">
               <h3 class="box-title">@if ( Auth::User()->language == 1 ) জার্নাল ক্রিয়েট @else Journal Create @endif</h3>
            </div>
            <div class="box-body">

               <form id="dynamic_form" action="{{ route("journallist-store-route") }}"  method="POST" onsubmit="return validate()" enctype="multipart/form-data" method="post" accept-charset="utf-8">
                @csrf
                  <div class="col-md-12 popup_details_div">
                     <div class="row">
                        <center>
                           <h3 class="page-title">@if ( Auth::User()->language == 1 ) জার্নাল ভাউচার @else Journal Voucher @endif</h3>
                        </center>
                     </div>
                     <div id='result'></div>
                     <br>
                  ...
                  <div class="row"style="margin-top: 15px" >
                     ...
                     <div class="col-md-4 text-right" >
                        <input type="submit" name="save_journal" id="submit" class="btn btn-flat bg-purple btn-sm" value="@if ( Auth::User()->language == 1 ) সেভ @else Save @endif"/> <a href="{{route('journal-list')}}" class="btn btn-flat bg-gray  ">@if ( Auth::User()->language == 1 ) ক্লোজ @else Close @endif</a>
                     </div>
                  </div>
               </form>
            </div>
...
<script>
   ...
   $(document).ready(function(){
    ...
    function dynamic_field(number)
    {
     ...
     html = '<tr>';
           html += '<th style="width:200px;"><select class="form-control select2" name="debit_sub_group_id[]" id="debit_sub_group_id'+count+'" onchange="find_debit_ledger('+count+')"><option value="">-Select-</option>@foreach($sub_groups as $subgroup)<option value="{{$subgroup['id']}}" @if(old('debit_sub_group_id') == $subgroup['id']) selected @endif>{{$subgroup['name']}}</option>@endforeach</select></th>';
           ...
           html += '<th style="width:200px;"><select class="form-control select2" name="credit_sub_group_id[]" id="credit_sub_group_id'+count+'" onchange="find_credit_ledger('+count+')"><option value="">-Select-</option>@foreach($sub_groups as $subgroup)<option value="{{$subgroup['id']}}" @if(old('credit_sub_group_id') == $subgroup['id']) selected @endif>{{$subgroup['name']}}</option>@endforeach</select></th>';
