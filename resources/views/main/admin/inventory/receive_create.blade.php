...
@endphp
    ...
    <h1>@if ( Auth::User()->language == 1 ) রিসিভ ক্রিয়েট @else Received Create @endif</h1>
    <ol class="breadcrumb">
        <li><a href="{{route('home')}}"><i class="fa fa-dashboard"></i>@if ( Auth::User()->language == 1 ) ড্যাসবোর্ড @else Dashboard @endif</a></li>
        <li class=""><a href="">@if ( Auth::User()->language == 1 ) রিসিভ ক্রিয়েট @else Received Create @endif</a></li>
        </ol>
    </section>
    <!-- Main content -->
    <section class="content">
        ...
        <h3 class="box-title">@if ( Auth::User()->language == 1 ) পারচেস প্রোডাক্ট রিসিভ @else Purchase Product Receive @endif</h3>
        </div>
        <div class="box-body">
        ...
        <label>@if ( Auth::User()->language == 1 ) পারচেস ইনভয়েস @else Purchase Invoice @endif</label>
        <select class="form-control select2" name="pur_invoice" id="pur_invoice" required>
        <option value="">-Select Invoice-</option>
        @foreach($purcha as $data)
          @if($data->quantity > $data->rqty)<option value="{{ $data->purchase_track }}">{{ $data->purchase_track }}</option>@endif
        @endforeach
        </select>
        </div>
        <div class="form-group">
        ...
        <input type="text" class="form-control datetimepicker" name="rcv_date" id="rcv_date" value="{{today()}}" placeholder="Receive Date" autocomplete="off">
        </div>
        </div>
        </div>
        <div class="col-md-8">
        ...
        <form enctype="multipart/form-data" method="POST" action="{{ route('product-stock-store') }}" accept-charset="utf-8">
            ...
            <input type="hidden" value="{{ $rcv_date }}" name="rcv_date">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><input type="text" name="rqty[]" id="rqty" value="{{ $data->rqty }}" class="form-control" readonly/></td>
                <td><input type="text" name="rcv_qty[]" id="rcv_qty" class="form-control" value="0"></td>
                <td><input type="text" name="rest_qty[]" id="rest_qty" value="{{ $data->quantity - $data->rqty}}" class="form-control" readonly></td>
            </tr>
            @endif
            @endforeach
        </tbody>
        ...
        <input type="submit" id="save_receiv" class="btn btn-flat bg-purple btn-sm " value="Receive"/> <a href="{{route('proreceive-list')}}" class="btn btn-flat bg-gray  ">Close</a>
        </form>
        @endif
        </div>
    </div>
